...
class UniSemester : public Semester {
    ...
    public:
        ...
        double getAverageCourseScoreInASemester() override {
            double result = 0;
            for (int i = 0; i < 4; i++) {
...
class StudentFactory {
    public:
        virtual unique_ptr<Student> createStudent(string, string, string, string *) = 0;
        virtual ~StudentFactory() {};
};

class UniStudentFactory : public StudentFactory {
    public:
        unique_ptr<Student> createStudent(string name, string dob, string school_name, string *course_name) override {
            return make_unique<UniStudent>(name, dob, school_name, course_name);
        }
};
...
class CollegeStudentFactory : public StudentFactory {
    public:
        unique_ptr<Student> createStudent(string name, string dob, string school_name, string *course_name) override {
            return make_unique<CollegeStudent>(name, dob, school_name, course_name);
        }
};
...
class University {
    ...
    public: 
        ...
        void addUniStu(const string& name, const string& dob, const string& school_name, string *course_name) {
            list.push_back(UniFactory->createStudent(name, dob, school_name, course_name));
            cout << "\n[" << name << "] " << "has been added on the list!\n";
        }
        void addCollegeStu(const string& name, const string& dob, const string& school_name, string *course_name) {
            list.push_back(CollegeFactory->createStudent(name, dob, school_name, course_name));
            cout << "\n[" << name << "] " << "has been added on the list!\n";
        }
        ...
        void display() {
            for (auto it = list.begin(); it != list.end(); it++) {
                cout << it - list.begin() + 1 << ". Stu name: " << (*it)->getname() << "; School's name: " << (*it)->getschoolname() << "; Institution: " << (*it)->getInstitution() << endl; 
                cout << "Student's score:" << endl;
                int sem_size = ((*it)->getInstitution() == "University") ? 8 : 4;
                for (int j = 0; j < sem_size; j++) {
                    cout << "Semester " << j + 1 << ":" << endl;
                    (*it)->Sem(j)->displayCourseScore();
                    cout << "Average score in Semester " << j + 1 << ": " << (*it)->Sem(j)->getAverageCourseScoreInASemester() << endl;
                    cout << endl;
                }
        ...
        void displaybestscore() {
            ...
            double maxscore = -1;
            for (auto &it : list) {
                maxscore = (maxscore < it->calculateAverageFinalScore()) ? it->calculateAverageFinalScore() : maxscore;
            }
            ...
            int idx = 1;
            for (auto &it: list) {
                if (it->calculateAverageFinalScore() == maxscore)
                    cout << left << setw(10) << idx++ << left << setw(38) << it->getname() << it->calculateAverageFinalScore() << endl; 
            }
        }

        void remove(const string& _name) {
            int cnt = 0;
            vector<unique_ptr<Student>> tmp;
            for (auto &it: list) {
                if (it->getname() == _name) {
                    cnt++;
                }
...
int main()
{
    ...
    unique_ptr<University> edu = make_unique<University>();
    ...
    while(1) {
        ...
        if (x == "1") {
            ...
            if (option == 1) edu->addUniStu(name, dob, school_name, course_name);
            else if (option == 2) edu->addCollegeStu(name, dob, school_name, course_name);
        }
        else if (x == "2") {
            string _name;
