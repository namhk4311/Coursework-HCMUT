...
class StudentFactory {
    public:
        virtual Student* createStudent(string, string, string, string *) = 0;
        virtual ~StudentFactory() {};
};

class UniStudentFactory : public StudentFactory {
    public:
        Student *createStudent(string name, string dob, string school_name, string *course_name) override {
            return new UniStudent(name, dob, school_name, course_name);
        }
};
...
class CollegeStudentFactory : public StudentFactory {
    public:
        Student *createStudent(string name, string dob, string school_name, string *course_name) override {
            return new CollegeStudent(name, dob, school_name, course_name);
        }
};
...
class University {
    private:
        vector<Student*> list;
        UniStudentFactory *UniFactory;
        CollegeStudentFactory *CollegeFactory;
    public: 
        University() {
            UniFactory = new UniStudentFactory();
            CollegeFactory = new CollegeStudentFactory();
        }
        int getlistsize() {
        ...
        void display() {
            ...
            for (it = list.begin(); it != list.end(); it++) {
                Student *stu = *it;
                cout << it - list.begin() + 1 << ". Stu name: " << stu->getname() << "; School's name: " << stu->getschoolname() << "; Institution: " << stu->getInstitution() << endl; 
                cout << "Student's score:" << endl;
                int sem_size = (stu->getInstitution() == "University") ? 8 : 4;
                for (int j = 0; j < sem_size; j++) {
                    cout << "Semester " << j + 1 << ":" << endl;
                    stu->Sem(j)->displayCourseScore();
                    cout << "Average score in Semester " << j + 1 << ": " << stu->Sem(j)->getAverageCourseScoreInASemester() << endl;
                    cout << endl;
                }
