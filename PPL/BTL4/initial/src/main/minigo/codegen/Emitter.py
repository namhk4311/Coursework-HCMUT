...
class Emitter():
    ...
    *    @param typ the type of the constant
    ...
    def emitALOAD(self, in_, frame):
        ...
        elif type(in_) in [cgen.ArrayType, cgen.ClassType, StringType, Id]:
    ...
    * @param in the type descriptor of the method.
    ...
    def emitINVOKEINTERFACE(self, lexeme, in_, frame):
        ...
        if not type(typ) is VoidType:
            frame.push()
        return self.jvm.emitINVOKEINTERFACE(lexeme, self.getJVMType(in_), str(len(typ.partype) + 1))
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    ...
    def emitABSTRACTMETHOD(self, lexeme, in_, isStatic=False):
        #lexeme: String
        #in_: Type
        #isStatic: Boolean
        #frame: Frame

        return self.jvm.emitMETHOD(f"abstract {lexeme}", self.getJVMType(in_), isStatic)
    
    def emitENDABSTRACTMETHOD(self):
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        elif type(in_) in [StringType, ArrayType, Id]:
            frame.pop()
    ...
    *   .super java/lang/Object<p>
    ...
    def emitPROLOG(self, name, parent, interface = False):
        ...
        result.append(self.jvm.emitCLASS(f"public {f"interface {name}" if interface else name}"))
